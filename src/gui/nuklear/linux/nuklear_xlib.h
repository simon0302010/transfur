...
NK_API struct nk_context *nk_xlib_init(XFont *, Display *, int scrn,
                                       Window root, Visual *vis, Colormap cmap,
                                       unsigned w, unsigned h);
#else
NK_API struct nk_context *nk_xlib_init(XFont *, Display *, int scrn,
                                       Window root, unsigned w, unsigned h);
#endif
...
NK_API struct nk_image nk_xsurf_load_image_from_memory(const void *membuf,
                                                       nk_uint membufSize);
...
NK_INTERN double nk_get_time(void) {
        struct timeval tv;
...
NK_INTERN unsigned long nk_color_from_byte(const nk_byte *c) {
        unsigned long res = 0;
...
NK_INTERN XSurface *nk_xsurf_create(int screen, unsigned int w,
                                    unsigned int h) {
        ...
        XSetLineAttributes(xlib.dpy, surface->gc, 1, LineSolid, CapButt,
                           JoinMiter);
        surface->drawable =
            XCreatePixmap(xlib.dpy, xlib.root, w, h,
                          (unsigned int)DefaultDepth(xlib.dpy, screen));
#ifdef NK_XLIB_USE_XFT
        surface->ftdraw =
            XftDrawCreate(xlib.dpy, surface->drawable, xlib.vis, xlib.cmap);
#endif
        return surface;
}

NK_INTERN void nk_xsurf_resize(XSurface *surf, unsigned int w, unsigned int h) {
        if (!surf)
                return;
        ...
        surf->drawable =
            XCreatePixmap(surf->dpy, surf->root, w, h,
...
NK_INTERN void nk_xsurf_scissor(XSurface *surf, float x, float y, float w,
                                float h) {
...
NK_INTERN void nk_xsurf_stroke_line(XSurface *surf, short x0, short y0,
                                    short x1, short y1,
                                    unsigned int line_thickness,
                                    struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
        XSetForeground(surf->dpy, surf->gc, c);
        XSetLineAttributes(surf->dpy, surf->gc, line_thickness, LineSolid,
                           CapButt, JoinMiter);
        XDrawLine(surf->dpy, surf->drawable, surf->gc, (int)x0, (int)y0,
                  (int)x1, (int)y1);
        XSetLineAttributes(surf->dpy, surf->gc, 1, LineSolid, CapButt,
                           JoinMiter);
}

NK_INTERN void nk_xsurf_stroke_rect(XSurface *surf, short x, short y,
...

        {
                ...
                XDrawLine(surf->dpy, surf->drawable, surf->gc, xc, y, xc + wc,
                          y);
                XDrawLine(surf->dpy, surf->drawable, surf->gc, x + w, yc, x + w,
                          yc + hc);
                XDrawLine(surf->dpy, surf->drawable, surf->gc, xc, y + h,
                          xc + wc, y + h);
                XDrawLine(surf->dpy, surf->drawable, surf->gc, x, yc, x,
                          yc + hc);
                ...
                XDrawArc(surf->dpy, surf->drawable, surf->gc, xc + wc - r,
                         yc + hc - r, (unsigned)r * 2, (unsigned)2 * r,
                         -90 * 64, 90 * 64);
        }
        XSetLineAttributes(surf->dpy, surf->gc, 1, LineSolid, CapButt,
                           JoinMiter);
...
NK_INTERN void nk_xsurf_fill_rect(XSurface *surf, short x, short y,
                                  unsigned short w, unsigned short h,
                                  unsigned short r, struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
        XSetForeground(surf->dpy, surf->gc, c);
        if (r == 0) {
...

        {
                ...
                XFillPolygon(surf->dpy, surf->drawable, surf->gc, pnts, 12,
                ...
                XFillArc(surf->dpy, surf->drawable, surf->gc, x, yc + hc - r,
                         (unsigned)r * 2, (unsigned)2 * r, 180 * 64, 90 * 64);
                XFillArc(surf->dpy, surf->drawable, surf->gc, xc + wc - r,
                         yc + hc - r, (unsigned)r * 2, (unsigned)2 * r,
                         -90 * 64, 90 * 64);
        }
}

NK_INTERN void nk_xsurf_fill_triangle(XSurface *surf, short x0, short y0,
        ...
        pnts[2].y = (short)y2;
        XSetForeground(surf->dpy, surf->gc, c);
        XFillPolygon(surf->dpy, surf->drawable, surf->gc, pnts, 3, Convex,
                     CoordModeOrigin);
}

NK_INTERN void nk_xsurf_stroke_triangle(XSurface *surf, short x0, short y0,
                                        short x1, short y1, short x2, short y2,
                                        unsigned short line_thickness,
                                        struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
...
NK_INTERN void nk_xsurf_fill_polygon(XSurface *surf,
                                     const struct nk_vec2i *pnts, int count,
                                     struct nk_color col) {
        int i = 0;
#define MAX_POINTS 128
        ...
        XFillPolygon(surf->dpy, surf->drawable, surf->gc, xpnts, count, Convex,
                     CoordModeOrigin);
#undef MAX_POINTS
}

NK_INTERN void nk_xsurf_stroke_polygon(XSurface *surf,
                                       const struct nk_vec2i *pnts, int count,
                                       unsigned short line_thickness,
                                       struct nk_color col) {
        int i = 0;
        ...
        XSetLineAttributes(surf->dpy, surf->gc, line_thickness, LineSolid,
                           CapButt, JoinMiter);
        for (i = 1; i < count; ++i)
                XDrawLine(surf->dpy, surf->drawable, surf->gc, pnts[i - 1].x,
                          pnts[i - 1].y, pnts[i].x, pnts[i].y);
        XDrawLine(surf->dpy, surf->drawable, surf->gc, pnts[count - 1].x,
                  pnts[count - 1].y, pnts[0].x, pnts[0].y);
        XSetLineAttributes(surf->dpy, surf->gc, 1, LineSolid, CapButt,
                           JoinMiter);
}

NK_INTERN void nk_xsurf_stroke_polyline(XSurface *surf,
                                        const struct nk_vec2i *pnts, int count,
                                        unsigned short line_thickness,
                                        struct nk_color col) {
        int i = 0;
        ...
        XSetForeground(surf->dpy, surf->gc, c);
        for (i = 0; i < count - 1; ++i)
                XDrawLine(surf->dpy, surf->drawable, surf->gc, pnts[i].x,
                          pnts[i].y, pnts[i + 1].x, pnts[i + 1].y);
        XSetLineAttributes(surf->dpy, surf->gc, 1, LineSolid, CapButt,
                           JoinMiter);
}

NK_INTERN void nk_xsurf_fill_circle(XSurface *surf, short x, short y,
                                    unsigned short w, unsigned short h,
                                    struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
        XSetForeground(surf->dpy, surf->gc, c);
...
NK_INTERN void nk_xsurf_stroke_circle(XSurface *surf, short x, short y,
                                      unsigned short w, unsigned short h,
                                      unsigned short line_thickness,
                                      struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
...
NK_INTERN void nk_xsurf_stroke_arc(XSurface *surf, short cx, short cy,
                                   unsigned short radius, float a_min,
                                   float a_max, unsigned short line_thickness,
                                   struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
        XSetLineAttributes(surf->dpy, surf->gc, line_thickness, LineSolid,
                           CapButt, JoinMiter);
        XSetForeground(surf->dpy, surf->gc, c);
        XDrawArc(surf->dpy, surf->drawable, surf->gc, (int)(cx - radius),
                 (int)(cy - radius), (unsigned)(radius * 2),
                 (unsigned)(radius * 2), (int)(a_min * 180 * 64 / NK_PI),
                 (int)(a_max * 180 * 64 / NK_PI));
}

NK_INTERN void nk_xsurf_fill_arc(XSurface *surf, short cx, short cy,
                                 unsigned short radius, float a_min,
                                 float a_max, struct nk_color col) {
        unsigned long c = nk_color_from_byte(&col.r);
        XSetForeground(surf->dpy, surf->gc, c);
        XFillArc(surf->dpy, surf->drawable, surf->gc, (int)(cx - radius),
                 (int)(cy - radius), (unsigned)(radius * 2),
                 (unsigned)(radius * 2), (int)(a_min * 180 * 64 / NK_PI),
                 (int)(a_max * 180 * 64 / NK_PI));
}

NK_INTERN void nk_xsurf_stroke_curve(XSurface *surf, struct nk_vec2i p1,
                                     struct nk_vec2i p2, struct nk_vec2i p3,
                                     struct nk_vec2i p4,
        ...
        for (i_step = 1; i_step <= num_segments; ++i_step) {
                ...
                nk_xsurf_stroke_line(surf, last.x, last.y, (short)x, (short)y,
                                     line_thickness, col);
                last.x = (short)x;
                last.y = (short)y;
        }
...
NK_INTERN void nk_xsurf_draw_text(XSurface *surf, short x, short y,
                                  const char *text, int len, XFont *font,
                                  struct nk_color cfg) {
        int tx, ty;
#ifdef NK_XLIB_USE_XFT
        XRenderColor xrc;
        ...
        XftDrawStringUtf8(surf->ftdraw, &color, font->ft, tx, ty,
                          (FcChar8 *)text, len);
        XftColorFree(surf->dpy, xlib.vis, xlib.cmap, &color);
#else
        XSetForeground(surf->dpy, surf->gc, fg);
        if (font->set)
                XmbDrawString(surf->dpy, surf->drawable, font->set, surf->gc,
                              tx, ty, (const char *)text, (int)len);
        else
                XDrawString(surf->dpy, surf->drawable, surf->gc, tx, ty,
                            (const char *)text, (int)len);
#endif
}

#ifdef NK_XLIB_INCLUDE_STB_IMAGE
NK_INTERN struct nk_image nk_stbi_image_to_xsurf(const void *filename_or_membuf,
                                                 nk_uint zero_or_membufSize) {
        ...
        if (zero_or_membufSize) {
                data = stbi_load_from_memory(
                    (const stbi_uc *)filename_or_membuf, zero_or_membufSize,
                    &width, &height, &actual_channels, channels);
        } else {
                data = stbi_load(filename_or_membuf, &width, &height,
        ...
        if (channels == 4 && actual_channels == 4) {
                ...
                aimage->clipMask =
                    XCreatePixmap(surf->dpy, surf->drawable, width, height, 1);

                if (aimage->clipMask) {
                        aimage->clipMaskGC =
                            XCreateGC(surf->dpy, aimage->clipMask, 0, 0);
                        XSetForeground(surf->dpy, aimage->clipMaskGC,
                                       BlackPixel(surf->dpy, surf->screen));
                        XFillRectangle(surf->dpy, aimage->clipMask,
                                       aimage->clipMaskGC, 0, 0, width, height);

                        XSetForeground(surf->dpy, aimage->clipMaskGC,
                                       WhitePixel(surf->dpy, surf->screen));
                        for (i = 0; i < isize; i += channels) {
                                ...
                                if (alpha > alpha_treshold)
                                        XDrawPoint(surf->dpy, aimage->clipMask,
                                                   aimage->clipMaskGC, x, y);
                        }
                }
        }

        aimage->ximage = XCreateImage(surf->dpy, CopyFromParent, depth, ZPixmap,
                                      0, (char *)data, width, height,
...
NK_INTERN void nk_xsurf_draw_image(XSurface *surf, short x, short y,
                                   unsigned short w, unsigned short h,
        ...
        if (aimage) {
                ...
                XPutImage(surf->dpy, surf->drawable, surf->gc, aimage->ximage,
                          0, 0, x, y, w, h);
...
NK_API void nk_xsurf_image_free(struct nk_image *image) {
        ...
        if (aimage->clipMaskGC)
                XFreeGC(surf->dpy, aimage->clipMaskGC);
        free(aimage);
}

NK_INTERN void nk_xsurf_clear(XSurface *surf, unsigned long color) {
        XSetForeground(surf->dpy, surf->gc, color);
        XFillRectangle(surf->dpy, surf->drawable, surf->gc, 0, 0, surf->w,
                       surf->h);
}

NK_INTERN void nk_xsurf_blit(Drawable target, XSurface *surf, unsigned int w,
                             unsigned int h) {
        XCopyArea(surf->dpy, surf->drawable, target, surf->gc, 0, 0, w, h, 0,
                  0);
}

NK_INTERN void nk_xsurf_del(XSurface *surf) {
#ifdef NK_XLIB_USE_XFT
...
NK_API XFont *nk_xfont_create(Display *dpy, const char *name) {
        ...
        if (font->set) {
                ...
                while (n--) {
                        ...
                        font->descent =
                            NK_MAX(font->descent, (*xfonts)->descent);
                        xfonts++;
                }
        } else {
...
NK_INTERN float nk_xfont_get_text_width(nk_handle handle, float height,
                                        const char *text, int len) {
...
NK_API struct nk_context *nk_xlib_init(XFont *xfont, Display *dpy, int screen,
                                       Window root,
#ifdef NK_XLIB_USE_XFT
        ...
        /* create invisible cursor */
        {
                ...
                if (blank == None)
                        return 0;
                xlib.cursor = XCreatePixmapCursor(dpy, blank, blank, &dummy,
                                                  &dummy, 0, 0);
...
NK_API void nk_xlib_set_font(XFont *xfont) {
        struct nk_user_font *font = &xfont->handle;
        font->userdata = nk_handle_ptr(xfont);
        font->height = (float)xfont->height;
        font->width = nk_xfont_get_text_width;
...
NK_API void nk_xlib_paste(nk_handle handle, struct nk_text_edit *edit) {
        ...
        NK_ASSERT(edit != &xlib.ctx.text_edit &&
                  "Paste not supported for temporary editors");
        xlib.clipboard_target = edit;
        /* Request the contents of the primary buffer */
        XConvertSelection(xlib.dpy, XA_PRIMARY, XA_STRING, XA_PRIMARY,
                          xlib.root, CurrentTime);
}
...
NK_API void nk_xlib_copy(nk_handle handle, const char *str, int len) {
        ...
        if (xlib.clipboard_data) {
                ...
                XSetSelectionOwner(xlib.dpy, XA_PRIMARY, xlib.root,
                                   CurrentTime);
                XSetSelectionOwner(xlib.dpy, xlib.xa_clipboard, xlib.root,
                                   CurrentTime);
        }
}

NK_API int nk_xlib_handle_event(Display *dpy, int screen, Window win,
                                XEvent *evt) {
        ...
        if (evt->type == KeyPress || evt->type == KeyRelease) {
                ...
                KeySym *code = XGetKeyboardMapping(
                    xlib.surf->dpy, (KeyCode)evt->xkey.keycode, 1, &ret);
                if (*code == XK_Shift_L || *code == XK_Shift_R)
                        nk_input_key(ctx, NK_KEY_SHIFT, down);
                else if (*code == XK_Control_L || *code == XK_Control_R)
                        nk_input_key(ctx, NK_KEY_CTRL, down);
                else if (*code == XK_Delete)
                        nk_input_key(ctx, NK_KEY_DEL, down);
                else if (*code == XK_Return || *code == XK_KP_Enter)
                ...
                else if (*code == XK_Alt_L || *code == XK_Alt_R)
                        ...
                        if (*code == 'c' && (evt->xkey.state & ControlMask))
                                ...
                                if (insert_toggle) {
                                        nk_input_key(
                                            ctx, NK_KEY_TEXT_INSERT_MODE, down);
                                } else {
                                        nk_input_key(ctx,
                                ...
                                if (down) {
                                        char buf[32];
                                        KeySym keysym = 0;
                                        if (XLookupString((XKeyEvent *)evt, buf,
                                                          32, &keysym,
                                                          NULL) != NoSymbol)
                ...
                if (evt->xbutton.button == Button1) {
                        if (down) { /* Double-Click Button handler */
                                float dt =
                                    nk_get_time() - xlib.last_button_click;
                                if (dt > NK_X11_DOUBLE_CLICK_LO &&
                                    dt < NK_X11_DOUBLE_CLICK_HI)
                ...
                if (ctx->input.mouse.grabbed) {
                        ...
                        XWarpPointer(xlib.dpy, None, xlib.surf->root, 0, 0, 0,
                                     0, (int)ctx->input.mouse.pos.x,
                                     (int)ctx->input.mouse.pos.y);
                }
                return 1;
                ...
                if (reply.xselection.target == xlib.xa_targets) {
                        ...
                        XChangeProperty(evt->xselection.display,
                                        evt->xselectionrequest.requestor,
                                        reply.xselection.property, XA_ATOM, 32,
                                        PropModeReplace,
                                        (unsigned char *)&target_list, 4);
                } else if (xlib.clipboard_data &&
                           (reply.xselection.target == xlib.xa_text ||
                            reply.xselection.target == xlib.xa_utf8_string ||
                        ...
                        XChangeProperty(evt->xselection.display,
                                        ...
                                        PropModeReplace,
                                        (unsigned char *)xlib.clipboard_data,
                                        xlib.clipboard_len);
                }
                XSendEvent(evt->xselection.display,
                           evt->xselectionrequest.requestor, True, 0, &reply);
...

                {
                        ...
                        do {
                                XGetWindowProperty(
                                    dpy, win, XA_PRIMARY, (int)pos, 1024, False,
                                    AnyPropertyType, &actual_type,
                                    &actual_format, &len, &remain, &data);
                                if (len && data)
                                        nk_textedit_text(xlib.clipboard_target,
                                                         (char *)data,
                                                         (int)len);
                                if (data != 0)
                                        XFree(data);
                                pos +=
                                    (len * (unsigned long)actual_format) / 32;
                        } while (remain != 0);
                }
                return 1;
        }
...
NK_API void nk_xlib_render(Drawable screen, struct nk_color clear) {
        ...
        nk_foreach(cmd, &xlib.ctx) {
                ...
                case NK_COMMAND_LINE: {
                        ...
                        nk_xsurf_stroke_line(surf, l->begin.x, l->begin.y,
                                             l->end.x, l->end.y,
                ...
                case NK_COMMAND_RECT: {
                        ...
                        nk_xsurf_stroke_rect(
                            surf, r->x, r->y,
                            NK_MAX(r->w - r->line_thickness, 0),
                            NK_MAX(r->h - r->line_thickness, 0),
                ...
                case NK_COMMAND_TRIANGLE_FILLED: {
                        ...
                        nk_xsurf_fill_triangle(surf, t->a.x, t->a.y, t->b.x,
                                               t->b.y, t->c.x, t->c.y,
                                               t->color);
                } break;
                ...
                case NK_COMMAND_TEXT: {
                        ...
                        nk_xsurf_draw_text(surf, t->x, t->y,
                ...
                case NK_COMMAND_CURVE: {
                        ...
                        nk_xsurf_stroke_curve(surf, q->begin, q->ctrl[0],
                                              q->ctrl[1], q->end, 22,
                ...
                case NK_COMMAND_IMAGE: {
                        ...
                        nk_xsurf_draw_image(surf, i->x, i->y, i->w, i->h,
                                            i->img, i->col);
                } break;
