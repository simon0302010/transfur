...
NK_API void nk_sdl_render(enum nk_anti_aliasing AA) {
...

        {
                ...
                sdl.ctx.delta_time_seconds =
                    (float)(now - sdl.time_of_last_frame) / 1000;
                ...
                nk_draw_foreach(cmd, &sdl.ctx, &dev->cmds) {
...

                        {
                                ...
                                SDL_RenderGeometryRaw(
                                    ...
                                    (const float *)((const nk_byte *)vertices +
                                                    vp),
                                    vs,
                                    (const SDL_Color *)((const nk_byte *)
                                                            vertices +
                                                        vc),
                                    vs,
                                    (const float *)((const nk_byte *)vertices +
                                                    vt),
                                    vs, (vbuf.needed / vs), (void *)offset,
                                    cmd->elem_count, 2);
...
NK_API struct nk_context *nk_sdl_init(SDL_Window *win, SDL_Renderer *renderer) {
        ...
        /* warn for cases where NK_SDL_CLAMP_CLIP_RECT should have been set but
         * isn't */
        SDL_GetRendererInfo(renderer, &info);
        SDL_GetVersion(&runtimeVer);
        if (strncmp("metal", info.name, 5) == 0 &&
            SDL_VERSIONNUM(runtimeVer.major, runtimeVer.minor,
                           runtimeVer.patch) < SDL_VERSIONNUM(2, 0, 22)) {
                SDL_LogWarn(
                    SDL_LOG_CATEGORY_APPLICATION,
                    "renderer is using Metal API but runtime SDL version "
                    "%d.%d.%d is older than compiled version %d.%d.%d, "
...
NK_API int nk_sdl_handle_event(SDL_Event *evt) {
        ...
        case SDL_KEYDOWN: {
                ...
                case SDLK_INSERT:
                        ...
                        if (insert_toggle) {
                                nk_input_key(ctx, NK_KEY_TEXT_INSERT_MODE,
                                             down);
                        } else {
                                nk_input_key(ctx, NK_KEY_TEXT_REPLACE_MODE,
                                             down);
                        }
                        break;
                case SDLK_a:
        ...
        case SDL_MOUSEWHEEL:
                nk_input_scroll(
                    ctx, nk_vec2(evt->wheel.preciseX, evt->wheel.preciseY));
                return 1;
        }
        return 0;
}
