...
NK_API struct nk_context *nk_d3d9_init(IDirect3DDevice9 *device, int width,
                                       int height);
NK_API void nk_d3d9_font_stash_begin(struct nk_font_atlas **atlas);
NK_API void nk_d3d9_font_stash_end(void);
NK_API int nk_d3d9_handle_event(HWND hwnd, UINT msg, WPARAM wparam,
                                LPARAM lparam);
...
struct nk_d3d9_vertex {
        /* D3d9 FFP requires three coordinate position, but nuklear writes only
           2 elements projection matrix doesn't use z coordinate => so it can be
           any value. Member order here is important! Do not rearrange them! */
        float position[3];
        nk_uchar col[4];
        float uv[2];
};
...
NK_API void nk_d3d9_create_state() {
        HRESULT hr;
        ...
        /* vertex format */
        IDirect3DDevice9_SetFVF(d3d9.device,
                                D3DFVF_XYZ + D3DFVF_DIFFUSE + D3DFVF_TEX1);

        /* blend state */
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_SRCBLEND,
                                        D3DBLEND_SRCALPHA);
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_DESTBLEND,
                                        D3DBLEND_INVSRCALPHA);
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_ALPHABLENDENABLE,
                                        TRUE);
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_BLENDOP,
                                        D3DBLENDOP_ADD);
        ...
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_CULLMODE,
                                        D3DCULL_NONE);
        IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_SCISSORTESTENABLE,
                                        TRUE);

        /* sampler state */
        IDirect3DDevice9_SetSamplerState(d3d9.device, 0, D3DSAMP_ADDRESSU,
                                         D3DTADDRESS_CLAMP);
        IDirect3DDevice9_SetSamplerState(d3d9.device, 0, D3DSAMP_ADDRESSV,
                                         D3DTADDRESS_CLAMP);
        IDirect3DDevice9_SetSamplerState(d3d9.device, 0, D3DSAMP_MAGFILTER,
                                         D3DTEXF_LINEAR);
        IDirect3DDevice9_SetSamplerState(d3d9.device, 0, D3DSAMP_MINFILTER,
                                         D3DTEXF_LINEAR);

        /* texture stage state */
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_COLOROP,
                                              D3DTOP_MODULATE);
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_COLORARG1,
                                              D3DTA_TEXTURE);
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_COLORARG2,
                                              D3DTA_DIFFUSE);
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_ALPHAOP,
                                              D3DTOP_MODULATE);
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_ALPHAARG1,
                                              D3DTA_TEXTURE);
        IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_ALPHAARG2,
                                              D3DTA_DIFFUSE);
...
NK_API void nk_d3d9_render(enum nk_anti_aliasing AA) {
        HRESULT hr;
        ...
        /* projection matrix */
        IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_PROJECTION,
                                      &d3d9.projection);
        ...
        /* convert from command queue into draw list and draw to screen */
        {
                ...
                NK_STORAGE const struct nk_draw_vertex_layout_element
                    vertex_layout[] = {
                        {NK_VERTEX_POSITION, NK_FORMAT_FLOAT,
                         NK_OFFSETOF(struct nk_d3d9_vertex, position)},
                        {NK_VERTEX_COLOR, NK_FORMAT_B8G8R8A8,
                         NK_OFFSETOF(struct nk_d3d9_vertex, col)},
                        {NK_VERTEX_TEXCOORD, NK_FORMAT_FLOAT,
                         NK_OFFSETOF(struct nk_d3d9_vertex, uv)},
                ...
                vertex_count =
                    (UINT)vbuf.needed / sizeof(struct nk_d3d9_vertex);
                ...
                nk_draw_foreach(cmd, &d3d9.ctx, &d3d9.cmds) {
                        ...
                        hr = IDirect3DDevice9_SetTexture(
                            d3d9.device, 0,
                            (IDirect3DBaseTexture9 *)cmd->texture.ptr);
                        NK_ASSERT(SUCCEEDED(hr));

                        scissor.left = (LONG)cmd->clip_rect.x;
                        scissor.right =
                            (LONG)(cmd->clip_rect.x + cmd->clip_rect.w);
                        scissor.top = (LONG)cmd->clip_rect.y;
                        scissor.bottom =
                            (LONG)(cmd->clip_rect.y + cmd->clip_rect.h);

                        hr = IDirect3DDevice9_SetScissorRect(d3d9.device,
                                                             &scissor);
                        NK_ASSERT(SUCCEEDED(hr));

                        NK_ASSERT(sizeof(nk_draw_index) == sizeof(NK_UINT16));
                        hr = IDirect3DDevice9_DrawIndexedPrimitiveUP(
                            d3d9.device, D3DPT_TRIANGLELIST, 0, vertex_count,
                            cmd->elem_count / 3, offset, D3DFMT_INDEX16,
...
static void nk_d3d9_get_projection_matrix(int width, int height,
                                          float *result) {
        const float L = 0.5f;
...
NK_API void nk_d3d9_release(void) { IDirect3DTexture9_Release(d3d9.texture); }

static void nk_d3d9_create_font_texture() {
        int w, h, y;
        const void *image;
        ...
        hr = IDirect3DDevice9_CreateTexture(
            d3d9.device, w, h, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8,
            D3DPOOL_DEFAULT, &d3d9.texture, NULL);
        NK_ASSERT(SUCCEEDED(hr));
        ...
        nk_font_atlas_end(&d3d9.atlas, nk_handle_ptr(d3d9.texture),
                          &d3d9.tex_null);
}
...
NK_API int nk_d3d9_handle_event(HWND wnd, UINT msg, WPARAM wparam,
                                LPARAM lparam) {
        ...
        case WM_SYSKEYUP: {
                ...
                case VK_LEFT:
                        if (ctrl)
                                nk_input_key(&d3d9.ctx, NK_KEY_TEXT_WORD_LEFT,
                                             down);
                        else
                ...
                case VK_RIGHT:
                        if (ctrl)
                                nk_input_key(&d3d9.ctx, NK_KEY_TEXT_WORD_RIGHT,
                                             down);
                        else
                ...
                case VK_INSERT:
                         ...
                         * kind of confusing since we have to negate it but
                         * we're already hacking it since Nuklear treats them as
                         * two separate keys rather than a single toggle state
                         */
                        if (!down) {
                                insert_toggle = !insert_toggle;
                                if (insert_toggle) {
                                        nk_input_key(&d3d9.ctx,
                                                     NK_KEY_TEXT_INSERT_MODE,
                                                     !down);
                                        /* nk_input_key(&d3d9.ctx,
                                         * NK_KEY_TEXT_REPLACE_MODE, down); */
                                } else {
                                        nk_input_key(&d3d9.ctx,
                                                     NK_KEY_TEXT_REPLACE_MODE,
                                                     !down);
                                        /* nk_input_key(&d3d9.ctx,
                                         * NK_KEY_TEXT_INSERT_MODE, down); */
                                }
                        }
                        return 1;
                ...
                case 'A':
                        if (ctrl) {
                                nk_input_key(&d3d9.ctx, NK_KEY_TEXT_SELECT_ALL,
                                             down);
                                return 1;
                        }
                        break;
                ...
                case 'B':
                        if (ctrl) {
                                nk_input_key(&d3d9.ctx, NK_KEY_TEXT_LINE_START,
                                             down);
                                return 1;
                        }
                        break;
                ...
                case 'E':
                        if (ctrl) {
                                nk_input_key(&d3d9.ctx, NK_KEY_TEXT_LINE_END,
                                             down);
                                return 1;
                        }
                        break;
        ...
        case WM_LBUTTONDOWN:
                nk_input_button(&d3d9.ctx, NK_BUTTON_LEFT,
        ...
        case WM_RBUTTONDOWN:
                nk_input_button(&d3d9.ctx, NK_BUTTON_RIGHT,
                                (short)LOWORD(lparam), (short)HIWORD(lparam),
                                1);
                SetCapture(wnd);
                return 1;
        ...
        case WM_MBUTTONDOWN:
                nk_input_button(&d3d9.ctx, NK_BUTTON_MIDDLE,
                                (short)LOWORD(lparam), (short)HIWORD(lparam),
                                1);
                SetCapture(wnd);
                return 1;
        ...
        case WM_XBUTTONDOWN:
                ...
                case XBUTTON1:
                        nk_input_button(&d3d9.ctx, NK_BUTTON_X1,
                                        (short)LOWORD(lparam),
                                        (short)HIWORD(lparam), 1);
                        break;
                case XBUTTON2:
                        nk_input_button(&d3d9.ctx, NK_BUTTON_X2,
                                        (short)LOWORD(lparam),
                                        (short)HIWORD(lparam), 1);
                        break;
                }
        ...
        case WM_MOUSEMOVE:
                nk_input_motion(&d3d9.ctx, (short)LOWORD(lparam),
                                (short)HIWORD(lparam));
                return 1;
...
static void nk_d3d9_clipboard_paste(nk_handle usr, struct nk_text_edit *edit) {
        HGLOBAL mem;
        SIZE_T size;
        LPCWSTR wstr;
        int utf8size;
        ...
        utf8size = WideCharToMultiByte(
            CP_UTF8, 0, wstr, (int)size / sizeof(wchar_t), NULL, 0, NULL, NULL);
        if (utf8size) {
...
static void nk_d3d9_clipboard_copy(nk_handle usr, const char *text, int len) {
        int wsize;
        ...
        if (wsize) {
                HGLOBAL mem =
                    GlobalAlloc(GMEM_MOVEABLE, (wsize + 1) * sizeof(wchar_t));
                if (mem) {
